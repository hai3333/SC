        ...
        <div class="row-fluid">
                    ...
                    <tbody>
						...
						%>				
					                            ...
					                            %>			<td><%=chW.get(j).getCh_checkTime() %></td><!-- 此时为null -->
					                            			...
					                            			<td><a href="/sc/teacher/Download?Wno=<%=Wno%>&c_id=<%=c_id%>&Uptime=<%=chW.get(j).getCh_upTime().toString().substring(0,chW.get(j).getCh_upTime().toString().length()-2) %>">下载</a></td>
					                            <%	}
					                            		else{
					                            %>			
			...
			<!-- 批改面板 -->
                <div class="panel-check" style="background-color: #F0F0F0;width: 100%;height: 100%; position: absolute;display: none;">
                	<form action="/sc/teacher/t_check" method = "post">
                		<!-- 隐藏表单传递批改号 -->
